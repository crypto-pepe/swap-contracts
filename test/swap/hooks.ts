...
import { setContract as setSwapContract } from '../../steps/swap';
...
export const mochaHooks = async (): Promise<Mocha.RootHookObject> => {
  return {
    async beforeAll(this: Mocha.Context) {
      ...
      // Deploy wrapped_token_bridge
      contracts.push(
        // eslint-disable-next-line prettier/prettier
        await deployMultisigContract(init_contracts, 'concentrated_swap', rootSeed)
      );
      // eslint-disable-next-line prettier/prettier
      setSwapContract(contracts.filter((f) => f.name == 'concentrated_swap')[0]);

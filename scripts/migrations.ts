...
(async () => {
  ...
  switch (options.network) {
    case 'mainnet':
      migrationsDir += 'mainnet/';
      break;
    case 'testnet':
      migrationsDir += 'testnet/';
      break;
    case 'custom':
  ...
  const network: NetworkConfig = getEnvironmentByName(options.network).network;
        ...
        .reduce(
          async (migrationPromise, migration) => {
            ...
            return new Promise<void>((resolve) => {
              console.log('Deploying migration ' + migration);
